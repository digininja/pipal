...
class Basic_Checker < Checker
	...
	def get_results()
		...
		0.upto(@lengths.count) do |len|
			...
			ret_str << "#{len.to_s} = #{@lengths[len].to_s} (#{percentage.round(2).to_s}%)\n" if @lengths[len] > 0
		...
		length_ordered.each do |pair|
			ret_str << "#{pair[0].to_s} = #{pair[1].to_s} (#{pair[2].round(2).to_s}%)\n" if pair[1] > 0
		end
		ret_str << "\n"
		...
		ret_str << "\nOne to six characters = #{@one_to_six_chars.to_s} (#{((@ne_to_six_chars.to_f/@total_words_processed) * 100).round(2).to_s}%)\n"
